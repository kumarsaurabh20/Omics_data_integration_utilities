...
def get_unique_mols(df, unique_dict, use_mols):
    if len(df) == 1:
        ...
        unique_smiles_df = df.drop_duplicates(subset=['predicted_substrate_id',
                                                                 'predicted_product_smiles'], keep='first')
        for i in df.index:
...
def query_filtered_rxn_db(rxn_df, map_df, base_rules_df, Options):
    ...
    while recent_smarts_passed:
        filtered_df = filter_df_with_map(rxn_df, map_df, recent_smarts_passed,
...
def df_to_graph(df, use_pd_graph):
    ...
    """
    if use_pd_graph:
        G = nx.from_pandas_edgelist(df, 'predicted_substrate_id', 'predicted_product_id',
